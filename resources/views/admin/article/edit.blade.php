...
@section('content')
    <div class="page-content">
        <div class="container-fluid">
            ...
            <!-- end page title -->
            <form class="col-6" action="{{ route('article.update', $article->id) }}" method="post"
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="description" name="description" rows="2" placeholder="Nhập mô tả bài viết...">{{ $article->description }}</textarea>
                    @error('description')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-3 col-6">
                    <label for="status1" class="form-label">Danh mục<span class="text-danger">*</span></label>
                    <select name="category_id" class="form-select w-100" id="status1">
                        <option value="">Chọn danh mục</option>
                        @foreach ($listCategory as $status)
                            <option value="{{ $status->id }}"
                                {{ $article->category_id == $status->id ? 'selected' : '' }}>{{ $status->name }}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                        <span class="text-danger">{{ $message }}</span>
                ...
                <div class="mb-3 col-6">
                    ...
                    <select name="user_id" class="form-select w-100" id="status1">
                        ...
                        @foreach ($listUser as $status)
                            <option value="{{ $status->id }}" {{ $article->user_id == $status->id ? 'selected' : '' }}>
                                {{ $status->name }}</option>
                        @endforeach
                    </select>
                    @error('user_id')
